...
TEST_CASE("When default matrix is created --> 'matrix[10][5] = {{0.5}}'", "[matrix_init][default][reg]") {{
	//Arrange
	Matrix matrix;
  	vector < vector<float> >  expected_grid, matrix_grid;
  	expected_grid = vector < vector<float> >(10, vector<float>(5, 0.5));
	//Act
    matrix_grid = matrix.getGrid();
	//Assert
...
TEST_CASE("When matrix is created with matrix[5][3] = {{0}} --> matrix[5][3] = {{0}}", "[matrix_init][custom][smoke]") {{
	//Arrange
	vector < vector<float> > expected_grid, matrix_grid;
	expected_grid = vector < vector<float> >(5, vector<float>(3, 0));
...
TEST_CASE("When matrix is created with empty grid --> matrix[0][0] = {{0}}", "[matrix_init][empty][reg]") {{
	//Arrange
	vector < vector<float> >  empty_grid, expected_grid, matrix_grid;
	empty_grid = vector < vector<float> >(0, vector<float>(0, 0));

	Matrix matrix(empty_grid);

  	expected_grid = vector < vector<float> >(1, vector<float>(1, 0));
	//Act
	matrix_grid = matrix.getGrid();
...
TEST_CASE("When matrix is updated with matrix[15][15] = {{80}} --> a new matrix[15][15] = {{80}}", "[matrix_init][update][reg]") {{
  	// Arrange
  	vector < vector<float> > initial_grid, new_grid, matrix_grid;
  	initial_grid = vector < vector<float> >(1, vector<float>(1, -0));
  	new_grid = vector < vector<float> >(15, vector<float>(15, 80));
